...
## Generation of ManualQuantTable: Quant-Standards

```{r import_quant1_std}

  #import Quant1-values for Quant-Mix
  quant1 = read.csv("config_files/quant1_values.csv", header = T)
...
## Generation of ManualQuantTable: Additional calibration curves
...
if (addq_idx == 'yes') { 
  ...
  if (ncol(quant_1add) == 1) {
   ...
   # quant_1add = data.frame(Lettercode = c("Cit", "Ita", "PGA6"),
   #                         Letter_Derivate = c("Cit", "Ita", "PGA6"),
   #                        Quant_ext = c(83260, 83260, 14620))
   # 
   # write.csv(quant_1add, paste0(path_setup, set_input, "add_quant/", nick_addq), row.names = FALSE)
