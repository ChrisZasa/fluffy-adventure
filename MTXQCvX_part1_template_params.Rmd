...
```{r import_datafiles, tidy  =  TRUE, echo=FALSE} 
...
imp_error = 0
  ...
  #### Annotation files ####
    ...
    if (ncol(data_extracts) == 1) {
       data_ = read.csv(paste0(path_setup, set_input, se_idx), T, sep = ";")
  ...
  ## (iii) mz 73 intensities and (iv) total peak densities from Chromatof
    ...
    if (ca_idx == TRUE) {
     if (mm_ca_idx == TRUE) {
        ...
        cinacid  <-   read.csv(paste0(path_setup, set_input,'gc/', nick_ca), T, sep = ',')
        check_emptyfile(cinacid, FALSE, "InternalStandard.csv")
      } else {
        imp_error = imp_error + 1
        message("No internal standard for data input format defined!")
      }
    } else {
    ...
    if (alkane_idx == TRUE) {
      ...
      imp_error = imp_error + 1
      message("No alkane file defined for this input format!")
    }
    ...
    if (nick_73 != "none") {
      ...
      imp_error = imp_error + 1
      message("No file with m/z 73 values defined for this input format!")
    }
    ...
    if (mm_peaks_idx == TRUE) {
      ...
      imp_error = imp_error + 1
      message("No file defined for this input format!")
    }
...
## Data normalization
### Internal standard cinnamic acid

```{r cinacid_qc, tidy = TRUE}

if (mm_ca_idx == TRUE) {
  if (ca_idx == TRUE) { 
    
    cin_data = file_shaping(cinacid, shape = "wide", ann, type = "sample", mode = "area")[,c("File", "PeakArea", "Batch_Id")]
    cinacid_qc = gc_metric_calc(cin_data, title = "cinacid")
    
  } else {
   Batch_Id = unique(alk_qc$Batch_Id)
   cinacid_qc = data.frame(Batch_Id, qc_metric = rep(NA, length(Batch_Id)))
   write.csv(cinacid_qc, paste0(path_setup,'output/gc/qcmetric_cinacid.csv'), row.names = F)
  }
}

```

```{r cinacid_plot, echo = F, fig.cap = "Quantification of internal extraction standard", fig.width = 5, fig.height = 5}
  
  if (ca_idx == TRUE && mm_ca_idx == TRUE) {

    cinacid_statistics = read.csv(paste0(path_setup, 'output/gc/Cinacid_stats.csv'), TRUE)
    
    if (nrow(cinacid_statistics) != 0) {
      cin_f = c('below','within','above')
      cinacid_statistics$CA_eval = factor(cinacid_statistics$CA_eval, levels = cin_f) 
    
     ggplot(cinacid_statistics, aes(File, PeakArea, color = CA_eval)) +
        geom_point() +
        ...
        scale_color_manual(values = color_ca,
                          guide = guide_legend(title = 'Evaluation CinAacid')) +
    		ylab('PeakArea in (-)') +
...
### Sum of Area of annotated metabolites per file

```{r sumArea_qc, tidy = TRUE}
 
if (mm_peaks_idx == TRUE) {
  suma_data <-  file_shaping(data_area, shape = 'wide', ann, type = "sample", mode = "area")
  ...
  message("No SumOfArea file generated in Metmax parser!")
}
...
if (mm_73_idx != "none" && mm_peaks_idx == TRUE) {
  data_deriv = file_shaping(data_73, shape = 'long', ann, type = "sample", mode = "area")
  total_peaks = file_shaping(total_peaks, 'long', ann, type = "sample", mode = "area")
...
```{r check_mqt}
	...
	if (params$data == "maui") {
	  ...
	  batch_id_def = create_batchid(ann)
	  ...
	  if (length(batch_id_def) < length(file_names)) {
	    message("Please check your annotation file. Your annotation files contains less files derived
	      from different Batches, than you copied in the MTXQC project! Continue processing though!")
	  }
	  
	  if (length(batch_id_def) > length(file_names)) {
			message("Please check the number of ManualQuantTables.tsv in your MTXQC project. \n
                There are more batches defined in your annotation file, than tsv-files present. \n
...
```{r add_calcurves, fig.height = 5, fig.width = 5, fig.cap = "Additional Calibration curves"}
...
if (addq_idx == 'yes') { 
  ...
  df_peakareas <-  file_shaping(data_area, shape = 'wide', 
                                ...
                                complete_ann = TRUE, type = "addQ", mode = "area")
  ...
  if (addq_integration == TRUE) {
    #Duplication of mqt_addq for all batch ids
      req_sets = nb_id - length(addq_batchid)
      
      missing_ids = c(addq_batchid[!(addq_batchid %in% batch_id_def)], batch_id_def[!(batch_id_def %in%   addq_batchid)])
      
 
    trial = mqt_addq[rep(seq_len(nrow(mqt_addq)), req_sets),] 
    
    idx_batchid = which(grepl("Batch_Id", colnames(mqt_addq)))
    trial = trial[,-idx_batchid]
    
    #create Batch_Id vector that contains all Ids with n length of mqt_addq
    
    net = character()
    
    for (i in length(batch_id_def)) {
      new_batchid = rep(batch_id_def[1], nrow(mqt_addq))
      net = c(net, new_batchid)
    }

    unique(setready$Batch_Id)
  }
...
```{r cal_curves, tidy = TRUE, warning=FALSE}
    ...
    qcurve_top5_rsquare(cal_clean, path_setup, "yes") 
...
```{r calcurve_par, echo = TRUE, fig.height = 5, fig.width = 5, fig.cap = "Calibration curves: Nb. of data points."}

if (nrow(qc_calcurve != 0)) {
...
```{r pools_linrange, tidy = TRUE}
  ...
  data_exp = file_shaping(data_area, shape = 'wide', ann, FALSE, type = "sample", mode = "area")
...
```{r samples_range, echo = FALSE, fig.width = 5, fig.height = 5, fig.cap = "Distribution of data points regarding linear range of the calibration curve"}
 if (nrow(lin_cal != 0)) {
   if (addq_idx == "yes") {
    ...
    print(
      ...
      scale_fill_manual(values = color_linearity, 
    	...
    	ggtitle('Fraction of measurements regarding quantification curve: Qstd') +
    	coord_flip() +
    ...
    print(
      ...
      scale_fill_manual(values = color_linearity, 
                        guide = guide_legend(title = 'Evaluation')) +
    	ggtitle('Fraction of measurements regarding quantification curve: Qadd') +
    	coord_flip() +
    	facet_wrap(Origin ~ Batch_Id, scales = "free_y") +
    	xlab('Derivate') +
    	ylab('Fraction of data points (%)') +
...
```{r calc_quantity, tidy = TRUE}
  ...
  data_q_plot[is.na(data_q_plot)]  =  0
```
...
```{r quant_calc_start, tidy = TRUE}
  ...
  if (ca_idx == TRUE) {
    data_quant_calc = merge(data_quant_calc, cinacid_statistics[,c("File", "CA_fac", "CA_eval")])
  } else {
    message("No internal standard defined!")
  }
  ...
  if (ca_idx == TRUE) {
    data_quant_norm = normalisation_calc(data_quant_calc, 1)
  } else {
    data_quant_norm = normalisation_calc(data_quant_calc, 0)
  }
...
```{r corr_ca_sof, warning=FALSE}
  ...
  if (ca_idx == TRUE) {
    ...
    data_within = subset(data_quant_norm, data_quant_norm$CA_eval == 'within')
    
    data_within_sel = data_within[,c('Lettercode', 'Batch_Id','sumA_Conc_pmio','ca_Conc_pmio')]      
    ...
    data_wo_na_r  =  ddply(data_wo_na, c("Lettercode", "Batch_Id"), transform, 
                   adj_r_squared_corr = (summary(lm(sumA_Conc_pmio ~ ca_Conc_pmio)))$adj.r.squared)
...
```{r se_imp, tidy = TRUE}
  ...
  data_mid_all = file_shaping(data_mid, shape = 'long', ann, type = "sample", mode = "area")
...
```{r se_nacount, tidy = TRUE}
 ...
 # SE_nascore = mid_metric_calc(data_SE_na)
  ...
  n  =  length(unique(factor(SE_nascore$na_frac)))
```
...
## 3-Lowest of MID
```{r SE_low3A_noneval, tidy = TRUE}  
  #SamplePeakArea
 # SE_eval_samples = data_SE_na[,c("Lettercode","File", "Batch_Id" ,"SamplePeakArea")]
  #SE_eval_backups = data_SE_na[,c("Lettercode","File", "Batch_Id" ,"BackupPeakArea")]
  #SE_eval_low_stats = mid_metric_calc2(SE_eval_samples, SE_eval_backups)
...
```{r inc_imp, tidy = TRUE}
  ...
  data_incorporation = file_shaping(data_inc, shape = "wide", ann, type = "sample", mode = "inc")
  colnames(data_incorporation)[grepl('PeakArea', colnames(data_incorporation))] <- "Inc"
