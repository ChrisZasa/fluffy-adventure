...
---
title: `r paste("MTXQCvX2 part2 Postprocessing - PROJECTNAME ", params$analysis)`
---
# Summary
...
```{r inc_plot, fig.width=7, fig.height=5, fig.cap="13C-Incorporation."}
#MOD!
      ...
      print(
        ggplot(subset(li_stat, li_stat$Lettercode == var), 
            ...
            geom_errorbar(aes(ymin = mean_li - sd_li, ymax = mean_li + sd_li),
                      width = 0.25, position = position_dodge(0.9)) +
            facet_wrap(get(params$par2) ~ Mass_Pos, scales = "free_y") +
            theme_bw() +
            ggtitle(var) +
                scale_fill_mtxqc(mypal) +
            ylab("13C-Inc. in (-)") +
...
```{r LabQ, echo=FALSE, fig.cap="Labeled quantities", fig.width=7, fig.height=3}
