...
```{r quantities_plot, echo=FALSE, fig.width=6, fig.height=3, fig.cap="Absolute quantities."}
#MOD!  
          ...
          ggplot(subset(quant_ss_plain, quant_ss_plain$Lettercode == var), 
                 aes(factor(get(params$par1)), mean_Q, fill = get(params$par2))) +
                  ...
                  facet_wrap(~ get(params$par2), scales = "free_y", nrow = 1) +
                    ggtitle(paste0("Quantitative comparison: ", var)) +
          ...
          ggplot(subset(quant_ss_plain, quant_ss_plain$Lettercode == var), 
                 aes(factor(get(params$par1)), mean_Q, fill = get(params$par2))) +
                  ...
                  geom_errorbar(aes(ymin = mean_Q - sd_Q, ymax = mean_Q + sd_Q), 
                              width = 0.25, position = position_dodge2(0.9)) +
                  theme_bw() +
                  facet_wrap(~ get(params$par3), scales = "free_y") +
                    ggtitle(paste0("Quantitative comparison: ", var)) +
                  xlab("Labeling time in (min)") +
                  ylab("Norm. Quantity in (pmol/1e+6 cells)") +
                  scale_fill_manual(values = color_cl) +
...
```{r relative_areas, echo=FALSE, fig.height=3, fig.width=6, fig.cap="Peak area based analysis of metabolites without available calibration curves."}
#MOD!
...
```{r inc_plot, fig.width=6, fig.height=3, fig.cap="13C-Incorporation."}
#MOD!
      ...
      print(
        ggplot(subset(li_stat, li_stat$Lettercode == var), 
            ...
            geom_errorbar(aes(ymin = mean_li - sd_li, ymax = mean_li + sd_li),
                      width = 0.25, position = position_dodge(0.9)) +
            facet_wrap(get(params$par2) ~ Mass_Pos, scales = "free_y", nrow = 2) +
            theme_bw() +
 ...
 for (var in unique(data_combined$Lettercode)) {
   print(
     ...
     geom_errorbar(aes(ymin = mean_labQ - sd_labQ, ymax = mean_labQ + sd_labQ),
                  width = 0.25, position = position_dodge(0.9)) +
        facet_wrap(Lettercode ~ Mass_Pos, scales = "free_y", ncol = 3) +
        theme_bw() +
      	scale_fill_manual(values = color_cl) +
