...
```{r add_calcurves, fig.height = 3, fig.width =5, fig.cap = "Additional Calibration curves"}
...
```{r quant_calc_start, tidy = TRUE}
  #1- normalization SumOfArea
    ...
    if (file.exists(paste0(path_setup, 'output/gc/SumArea_stats.csv')) == TRUE) {
      ...
      data_quant_calc$sum_area = rep(1, length(data_quant_calc$File))
      data_quant_calc$area_fac = rep(1, length(data_quant_calc$File))
      
      message("WARNING: Sum of area normalisation factor set to value = 1 due to missing input.")
    }
  ...
  if (ca_idx == TRUE) {
    if (is_exit == 0) {
      ...
      data_quant_calc$CA_fac = rep(1, length(data_quant_calc$File))
      data_quant_calc$Ca_eval = rep(1, length(data_quant_calc$File))
      
      message("WARNING: Internal Standard normalisation factor set to value = 1 due to missing input.")
    }
  } else {
