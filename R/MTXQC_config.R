...
#conversion list    
con_se = read.csv("config_files/conversion_metabolite.csv", TRUE)

if (ncol(con_se) == 1) {
  con_se = read.csv("config_files/conversion_metabolite.csv", T, sep = ";")
...
pathway_profile  =  read.csv('config_files/letter_pathway_complete.csv', T)
...
mass_li = read.csv('config_files/incorp_calc_masses.csv',  T)
backup_mids = read.csv('config_files/mid_backups.csv', T)
