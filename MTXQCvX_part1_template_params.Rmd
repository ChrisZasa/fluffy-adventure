---
title: 'MTXQCvX2 - Part1: PROJECT TITLE'
author:
...
abstract: Please use this space for a short summary of the current state of the project.
---
...
#### import params ------------------XY_params.csv----in project-folder----------------------
  files_params = import_content(path_setup, "params")
  
  setup_params = files_params$MTXQC_params
  
  if (params$data == "maui") {
    id_settings = files_params$Maui_params
  } else {
    id_settings = files_params$Metmax_params
  }

#### import config_mtx -----------------------------------------

  files_mtx = import_content(dir = "config_mtx/", mode = "conf_mtx")
  
  #conversion table
  con_se = files_mtx$conversion_metabolite
   
    if (ncol(con_se) == 1) {
      con_se = read.csv("config_mtx/conversion_metabolite.csv", sep = ";")
      message("Warning: Colon-separated file detected!")
    }
  
  #Metabolic profile pathway list
  pathway_profile = files_mtx$letter_pathway_complete
  
  #reference file summarising mass of interest
  mass_li = files_mtx$incorp_calc_masses
  backup_mids = files_mtx$mid_backups


##### config_files -----------------------------------------------------------

  files_config = import_content(dir = "config_files/", mode = "params")

  file_spec = files_config$conv_filenames
  file_spec_manVal = files_config$conv_filenames_manVal
  
  source("R/Imports_part1.R")
...
```{r check_mqt}
	...
	if (params$data == "maui") {
	  ...
	  cual = check_mqt(ann, path_setup, mode = "maui")
    mqt_std = create_manualquanttable(cual, q1_values = quant_table, con_se, FALSE)
    message("ManualQuantTable for MAUI input generated and exported!")
	}

	if (params$data == "metmax") {
	  
	  cual = check_mqt(ann, path_setup, mode = "metmax")
		mqt_std = create_manualquanttable(cual, q1_values = quant_table, con_se, FALSE)
		message("ManualQuantTable for Metmax input generated and exported!")
	}
```
...
if (addq_idx == 'yes') { 
  ...
  samples_batchid = create_batchid(ann)
  
  if (length(addq_batchid) == length(samples_batchid)) {
    ...
    missing_ids = c(addq_batchid[!(addq_batchid %in% samples_batchid)], samples_batchid[!(samples_batchid %in%   addq_batchid)])
    message("Additional calibration curves are detected for the batches: ", addq_batchid) 
  }
  ...
  if (addq_integration == "yes") {
    ...
    merge_sets = data.frame(Batch_Id = samples_batchid, Origin = rep("Qadd", length(samples_batchid)))
...
```{r merge_mqts}
  ...
  if (addq_idx == 'yes') {
    ...
    mqt_addq = subset(mqt_addq, mqt_addq$Batch_Id %in% samples_batchid)
